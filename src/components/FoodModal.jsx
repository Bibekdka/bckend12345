    ...
    const submitReview = async () => {
        ...
        try {
            ...
            await runTransaction(db, async (transaction) => {
                const sfDoc = await transaction.get(foodRef);
                if (!sfDoc.exists()) {
                    throw "Document does not exist!";
                }

                const currentReviews = sfDoc.data().reviews || [];
                const newReviews = [...currentReviews, review];
                const newAvg = newReviews.reduce((a, b) => a + b.rating, 0) / newReviews.length;
            ...
            setComment("");
            // Optional: Close modal or just show success? Let's just clear comment to allow another or show it appeared.
        } catch (e) {
